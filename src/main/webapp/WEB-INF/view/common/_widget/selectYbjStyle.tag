...
<input type="hidden" id="_${x.index!}" data-type="selectYbjStyle" name="token_${table!x.table}.${x.index!}" value="${x.value!}" />
	...
	$(function(){
		$("#_${x.index!}_INPUT").bind("click",function(){
			...
			layer.open({
        	    ...
        	    fix: false, //不固定
        	    maxmin: true,
        	    content: "${basePath}/yangbanjianStyle/open/"
		...
		if($("#_${x.index!}").val() != ""){
			initOpenYbjStyle($("#_${x.index!}").val());
		}
	});
	function initOpenYbjStyle(val){
		$.post("${basePath}/yangbanjianStyle/selectName",{source:"${x.source!}", type:"${x.type!}", where:"${x.where!}", intercept:"${x.intercept!}", val:val},function(data){
			if(data.code === 0){
