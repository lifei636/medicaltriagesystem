...
===
UPDATE patient_queue SET state_patient=8,is_display=2 where id=#{id}

updateFZbyscan
===
UPDATE patient_queue SET state_patient=2,call_count=0,opr_time=#{opr_time} where id=#{id}
...
UNION ALL
	...
	FROM
		(
			...
			WHERE
				#a.state_patient in (0,2,3,4,6,7,50)
				a.state_patient NOT IN (1,2,5,8,51, 52, 53, 54)
			AND b.queue_type_id =#{queue_type_id}
			AND a.is_display = 2
			and a.is_deleted=0
			and a.time_interval in (0,1,2)
			ORDER BY
				a.time_interval,
				a.register_id + '' ASC,
				a.opr_time,
...
INNER JOIN doctor c ON c.doctor_id = b.doctor_id
INNER JOIN pager d ON d.doctor_id = c.doctor_id
SET a.state_patient = 53,a.opr_time=#{opr_time},a.call_time=DATE_FORMAT(NOW(),'%Y-%m-%d %H:%m:%s')
WHERE
	a.id = #{patient_queue_id} 
AND d.ip = #{pager_ip}
AND c.login_id = #{login_id}
...
INNER JOIN doctor c ON c.doctor_id = b.doctor_id
INNER JOIN pager d ON d.doctor_id = c.doctor_id
SET a.state_patient = 54,a.opr_time=#{opr_time},a.call_count=a.call_count+1
WHERE
	a.id = #{patient_queue_id}
AND d.ip =#{pager_ip}
AND c.login_id = #{login_id}
...
INNER JOIN doctor c ON c.doctor_id = b.doctor_id
INNER JOIN pager d ON d.doctor_id = c.doctor_id
SET a.state_patient = 1,a.opr_time=#{opr_time},a.call_time=DATE_FORMAT(NOW(),'%Y-%m-%d %H:%m:%s')
WHERE
	a.id = #{patient_queue_id}
AND d.ip =#{pager_ip}
AND c.login_id = #{login_id}
...
SELECT
	...
	h.content,
	h.caller,
	b.display_name AS zsmc,
	d.displayname AS jhqmc,
	h.time_interval,
	h.is_display
FROM
	doctor a
...
INNER JOIN queue_type b ON a.queue_type_id = b.queue_type_id
INNER JOIN triage c ON c.triage_id = b.triage_id
SET a.content = '2', a.caller = '2',a.callerip=#{call},a.caller2=#{call},a.caller3=#{call},a.caller4=#{call}
WHERE
	a.id = #{id} 
AND a.queue_type_id = #{queue_type_id}
AND c.ip = #{ip}
...
SET a.state_patient = #{status},a.content=#{call},a.caller=#{call},a.callerip=#{call},a.caller2=#{call},a.caller3=#{call},a.caller4=#{call}
WHERE
	a.patient_source_code=#{source_code}
AND c.ip = #{ip}
