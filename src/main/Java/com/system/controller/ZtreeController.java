...
public class ZtreeController extends BaseController {
	...
	public void getTreeListName() {
		...
		final Map<String, Object> modelOrMap = params;
		List<Map<String, Object>> list = CacheKit.get(DICT_CACHE, "ztree_list_" + type, new IDataLoader() {
			@Override
			public Object load() {
	...
	private String getSql(String type, String source) {
		...
		if (type.indexOf("dict") >= 0) {
			...
			sql = "select NUM as \"num\",ID as \"id\",PID as \"pId\",NAME as \"name\",(case when (pId=0 or pId is null) then 'true' else 'false' end) \"open\" from  TFW_DICT where code="
					+ code;
		...
		} else if (type.equals("GoodsCategory") || source.equals("GoodsCategory")) {
			sql = Blade.dao().getScript("GoodsCategory.ztree").getSql();
		} else if (type.equals("GoodsCategory1") || source.equals("GoodsCategory1")) {
			sql = Blade.dao().getScript("GoodsCategory.ztree1").getSql();
		} else {
