...
import com.core.jfinal.ext.render.excel.PoiRender;
...
public class ExcelController extends BaseController {
	...
	public void export() {
		...
		render(PoiRender.me(dataResult).fileName(menu_name + DateKit.getAllTime() + ".xls").headers(header)
				.sheetName(code).cellWidth(5000).headerRow(2));
	}

	private String getInfoByCode(String code, String col) {
		List<Map<String, Object>> menu = CacheKit.get("menuCache", "menu_table_all", new IDataLoader() {
			@Override
			public Object load() {
				return Db.init().selectList(
						"select CODE,PCODE,NAME,URL,SOURCE,PATH,TIPS,ISOPEN from TFW_MENU order by levels asc,num asc");
			}
		});
