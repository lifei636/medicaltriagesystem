...
public class Db {
	...
	public int save(String tableName, String pk, Record maps) {
		...
		String sqlTemplate = Func.format(mainSql, tableName, JStrKit.removeSuffix(fields.toString(), ","),
				JStrKit.removeSuffix(values.toString(), ","));
	...
	public <T> List<T> getList(String sqlTemplate, Class<?> clazz, Object paras, int pageIndex, int pageSize) {
		List<T> all = (List<T>) getSqlManager().execute(sqlTemplate, clazz, paras, (pageIndex - 1) * pageSize + 1,
				pageSize);
		return all;
	}
