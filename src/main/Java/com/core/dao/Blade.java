...
public class Blade {
	...
	public Record findOneColBy(String columns) {
		List<Map> list = getSqlManager().execute(getSelectSql(columns) + getFromSql(), Map.class, Record.create(), 1,
				1);
		if (list.size() == 0) {
			return null;
		} else {
	...
	public Record findOneColBy(String columns, String where, Object modelOrMap) {
		String sql = getSelectSql(columns) + getFromSql() + getWhere(where);
		List<Map> list = getSqlManager().execute(sql, Map.class, modelOrMap, 1, 1);
		if (list.size() == 0) {
			return null;
		} else {
			return Record.parse(list.get(0));
	...
	public List<Record> findColBy(String columns, String where, Object modelOrMap) {
		List<Map> list = getSqlManager().execute(getSelectSql(columns) + getFromSql() + getWhere(where), Map.class,
				modelOrMap);
		List<Record> maps = new ArrayList<>();
		for (Map m : list) {
			maps.add(Record.parse(m));
	...
	public <T> List<T> findBy(String where, Object modelOrMap) {
		List<T> models = (List<T>) getSqlManager().execute(getSelectSql() + getFromSql() + getWhere(where),
				this.modelClass, modelOrMap);
		return models;
	}
	...
	public <T> List<T> findBy(String columns, String where, Object modelOrMap) {
		List<T> models = (List<T>) getSqlManager().execute(getSelectSql(columns) + getFromSql() + getWhere(where),
				this.modelClass, modelOrMap);
		return models;
	}
	 ...
	 */
	/*
	 * public boolean saveAssign(Object model) { SQLManager sql = getSqlManager();
	 * String table = sql.getNc().getTableName(this.modelClass); ClassDesc desc =
	 * sql.getMetaDataManager().getTable(table).getClassDesc(this.modelClass,
	 * sql.getNc()); Method getterMethod =
	 * desc.getIdMethods().get(desc.getIdNames().get(0)); String name =
	 * getterMethod.getName(); String setterName = name.replaceFirst("get", "set");
	 * try{ Class target = this.modelClass; Method setterMethod =
	 * target.getMethod(setterName, new Class[]{getterMethod.getReturnType()});
	 * Object value = UUID.randomUUID().toString().replace("-", ""); value =
	 * ScalarHandler.convertValueToRequiredType(value,
	 * getterMethod.getReturnType()); setterMethod.invoke(model, new
	 * Object[]{value}); }catch(Exception ex){ throw new
	 * UnsupportedOperationException("autoAssignKey failure " + ex.getMessage()); }
	...
	public boolean update(Object model) {
		...
		if (Cst.me().isOptimisticLock()) {
			// 1.数据是否还存在
			String sqlExist = new StringBuffer("select * from ").append(table).append(" where ").append(pk)
	...
	public <T> List<T> getList(String sqlTemplate, Object paras, int start, int size) {
		List<T> all = (List<T>) getSqlManager().execute(sqlTemplate, this.modelClass, paras, (start - 1) * size + 1,
				size);
		return all;
	}
