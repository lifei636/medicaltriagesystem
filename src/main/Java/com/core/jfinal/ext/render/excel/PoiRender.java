package com.core.jfinal.ext.render.excel;
/**
...
import com.core.jfinal.ext.kit.excel.PoiExporter;
...
public class PoiRender extends Render {
	...
	public void render() {
		...
		try {
			...
			PoiExporter.data(data).version(version).sheetNames(sheetNames).headerRow(headerRow).headers(headers)
					.columns(columns).cellWidth(cellWidth).export().write(os);
		} catch (Exception e) {
