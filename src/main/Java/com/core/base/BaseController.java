...
import com.core.modules.support.BeanInjector;
import com.core.toolbox.Record;
import com.core.toolbox.grid.GridManager;
...
public class BaseController extends Controller implements Const, ConstShiro, ConstCurd, ConstCache {
	...
	private <T> T paraInject(Class<T> beanClass) {
		return BeanInjector.inject(beanClass, getRequest(), false);
	}

	private <T> T paraInject(Class<T> beanClass, Map<String, Object> switchMap) {
		return BeanInjector.inject(beanClass, switchMap, getRequest(), false);
	}

	private <T> T paraInject(Class<T> beanClass, String paraPerfix) {
		return BeanInjector.inject(beanClass, paraPerfix, getRequest(), false);
	}

	private <T> T paraInject(Class<T> beanClass, Map<String, Object> switchMap, String paraPerfix) {
		return BeanInjector.inject(beanClass, switchMap, paraPerfix, getRequest(), false);
	}
