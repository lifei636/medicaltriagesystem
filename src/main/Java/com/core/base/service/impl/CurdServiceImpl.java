...
public class CurdServiceImpl implements CurdService {
	...
	public boolean delByIds(BaseController ctrl, String ids, Class<?> modelClass, MetaIntercept intercept) {
		...
		boolean temp = Blade.create(modelClass).updateBy("status = #{status}", "id in (#{join(ids)})",
				Record.create().set("status", 5).set("ids", ids.split(",")));
		boolean tempAfter = true;
		if (null != intercept && temp) {
	...
	public boolean restoreByIds(BaseController ctrl, String ids, Class<?> modelClass, MetaIntercept intercept) {
		...
		boolean temp = Blade.create(modelClass).updateBy("status = #{status}", "id in (#{join(ids)})",
				Record.create().set("status", 1).set("ids", ids.split(",")));
		boolean tempAfter = true;
		if (null != intercept && temp) {
	...
	@Override
	public Object paginate(Integer page, Integer rows, String source, String para, String sort, String order,
			MetaIntercept intercept, BaseController ctrl) {
