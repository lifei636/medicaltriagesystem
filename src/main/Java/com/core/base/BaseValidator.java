...
public abstract class BaseValidator extends Validator {
	...
	protected void validateSql(String field, String errorMessage) {
		...
		sql = sql.toLowerCase();
		if (sql.indexOf("delete") >= 0 || sql.indexOf("update") >= 0 || sql.indexOf("insert") >= 0
				|| sql.indexOf("drop") >= 0) {
			addError(errorMessage);
		}
	}
	...
	protected void validateInteger(String field, int min, int max, String errorMessage) {
		validateIntegerValue(getController().getPara(field), min, max, errorMessage);
	}

	private void validateIntegerValue(String value, int min, int max, String errorMessage) {
	...
	private void validateLongValue(String value, long min, long max, String errorMessage) {
		if (StrKit.isBlank(value)) {
			addError(errorMessage);
			return;
		}
	...
	protected void validateDate(String field, String min, String max, String errorMessage) {
		...
		try {
			validateDate(field, DateKit.parseTime(Func.format(min)), DateKit.parseTime(Func.format(max)), errorMessage);
		} catch (Exception e) {
			addError(errorMessage);
		}
	}
	...
	protected void validateString(String field, int minLen, int maxLen, String errorMessage) {
		validateStringValue(getController().getPara(field), minLen, maxLen, errorMessage);
	}

	private void validateStringValue(String value, int minLen, int maxLen, String errorMessage) {
		if (StrKit.isBlank(value)) {
			addError(errorMessage);
			return;
		}
