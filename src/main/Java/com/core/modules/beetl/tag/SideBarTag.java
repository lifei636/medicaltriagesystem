		...
		try {
			...
			for (Map<String, Object> side : sideBar) {
				if (Func.toInt(side.get("LEVELS")) == 1) {
					...
					firstMenu += "<li >";
					firstMenu += "	<a data-url=\"" + href + "\" " + addtabs + " data-title=\"" + side.get("NAME")
							+ "\" data-icon=\"fa " + side.get("ICON") + "\" class=\""
							+ getDropDownClass(Func.format(side.get("CODE")), "dropdown-toggle") + " tmsp-pointer\">";
					firstMenu += "		<i class=\"menu-icon fa " + side.get("ICON") + "\"></i>";
					firstMenu += "		<span class=\"menu-text\">" + side.get("NAME") + "</span>";
					firstMenu += "		<b class=\"arrow "
							+ getDropDownClass(Func.format(side.get("CODE")), "fa fa-angle-down") + "\"></b>";
					firstMenu += "	</a>";
	...
	public String reloadMenu(List<Map<String, Object>> sideBar, String pCode, String pStr, int levels, String ctxPath) {
		...
		for (Map<String, Object> subside : sideBar) {
			...
			if ((_pCode.equals(pCode) && _levels > levels)) {
				...
				Str += "<li>";
				Str += "	<a data-url=\"" + href + "\" " + addtabs + " data-title=\"" + _name + "\" data-icon=\"fa "
						+ _icon + "\" class=\"" + getDropDownClass(_code, "dropdown-toggle") + " tmsp-pointer\">";
