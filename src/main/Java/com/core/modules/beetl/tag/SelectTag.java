...
import com.core.toolbox.kit.ClassKit;
	...
	public void render() {
		try {
			...
			if (type.equals("dict")) {
				...
				CACHE_NAME = ConstCache.PROVINCE;
				sql = "SELECT REGION_CODE AS ID,REGION_NAME AS TEXT FROM region  WHERE PARENT_ID =1";
				intercept = Cst.me().getDefaultSelectFactory().provinceIntercept();
			} else if (type.equals("city")) {
				CACHE_NAME = ConstCache.CITY;
				//
				sql = "SELECT REGION_CODE AS ID,REGION_NAME AS TEXT FROM region  WHERE PARENT_ID =(SELECT REGION_ID FROM region WHERE REGION_CODE="
						+ where + ")";
				intercept = Cst.me().getDefaultSelectFactory().CityIntercept();
				where = "";
			} else if (type.equals("area")) {
				CACHE_NAME = ConstCache.CITY;
				sql = "SELECT REGION_CODE AS ID,REGION_NAME AS TEXT FROM region  WHERE PARENT_ID =(SELECT REGION_ID FROM region WHERE REGION_CODE="
						+ where + ")";
				intercept = Cst.me().getDefaultSelectFactory().AreaIntercept();
				where = "";
			...
			List<Map<String, Object>> dict = CacheKit.get(CACHE_NAME,
					"dict_" + type + "_" + code + "_" + ShiroKit.getUser().getId(), new IDataLoader() {
						@Override
						public Object load() {
			...
			sb.append("<select onchange=\"" + sid + "_selectChanged('" + sid + "')\" " + required
					+ " class=\"form-control\" id=\"" + sid + "\"  name=\"" + token + name + "\">");
