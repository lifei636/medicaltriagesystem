 ...
 */
package com.core.modules.beetl.tag;

import com.common.vo.TreeNode;
...
public class DropDownTag extends Tag {
	...
	public void render() {
		try {
			...
			if (type.equals("dict")) {
				...
				sql = "select ID,(case when PID is null then 0 else PID end) PID,NUM,name as TEXT from  TMSP_DICT where code="
						+ code + " and num>0";
			...
			} else if (type.equals("ybjgroup")) {
				sql = "select id ID,id NUM,0 as PID,name as TEXT from yangbanjiangroup";
			} else if (type.equals("newstype")) {
				sql = "select id ID,id NUM,0 as PID,type_name as TEXT from newstype";
			}

			final String sqlstr = sql;

			List<Map<String, Object>> dict = CacheKit.get(ConstCache.DICT_CACHE, "dropdown_" + type + "_" + code,
					new IDataLoader() {
						@Override
						public Object load() {
	...
	private String getDropDownHtml(List<TreeNode> nodeList, String id, String name, Object value, int lev, String token,
			String required) {
		StringBuilder sb = new StringBuilder();
		sb.append("<div class=\"input-group-btn\">");
		sb.append(" <input type=\"hidden\" id=\"" + id + "\" " + required + " name=\"" + token + name + "\" value=\""
				+ value + "\"/>");
		...
		sb.append("  <li><a href=\"#\" tabindex=\"-1\" onclick=\"$('#_DROPDOWN" + id + "').html('请选择');$('#" + id
				+ "').val('');$('#form_token').val(1);$('#" + id + "').attr('name','" + name + "');\">清空</a></li>");
	...
	private String reloadMenu(TreeNode treeNode, int levels, String id, String name, int lev) {
		...
		if (treeNode.isParent() && parentSize == levels && lev > levels + 1) {
			...
			sm.append(
					"<li><a href=\"#\" tabindex=\"-1\" onclick=\"$('#_DROPDOWN" + id + "').html('" + treeNode.getName()
							+ "');$('#" + id + "').val('" + treeNode.getNum() + "');$('#form_token').val(1);$('#" + id
							+ "').attr('name','" + name + "');\">" + treeNode.getName() + "</a></li>");
		} else if (parentSize > 0) {
			if (parentSize == levels && lev >= levels + 1) {
				sm.append("<li><a href=\"#\" tabindex=\"-1\" onclick=\"$('#_DROPDOWN" + id + "').html('"
						+ treeNode.getName() + "');$('#" + id + "').val('" + treeNode.getNum()
						+ "');$('#form_token').val(1);$('#" + id + "').attr('name','" + name + "');\">"
						+ treeNode.getName() + "</a></li>");
			}
		} else {
			sm.append(
					"<li><a href=\"#\" tabindex=\"-1\" onclick=\"$('#_DROPDOWN" + id + "').html('" + treeNode.getName()
