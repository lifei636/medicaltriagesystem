 ...
 */
package com.core.modules.beetl;
...
import com.core.toolbox.kit.DateKit;
import com.core.toolbox.kit.DateTimeKit;
...
public class BeetlExt {
	...
	public String getRightMenu(final Object userId, Object roleId, final String code) {
		Map<String, Object> userRole = CacheKit.get(ConstCache.MENU_CACHE, "roleExt_" + userId, new IDataLoader() {
			@Override
			public Object load() {
				return Db.init().selectOne("select * from tfw_role_ext where userId=#{userId}",
		...
		sql.append(
				"	 and (url like '%add%' or url like '%edit%' or url like '%remove%'  or url like '%del%' or url like '%view%' ) ");
		...
		List<Map<String, Object>> btnList = CacheKit.get(ConstCache.MENU_CACHE,
				"btnList_rightMenu_" + code + "_" + userId, new IDataLoader() {
					@Override
					public Object load() {
		...
		for (Map<String, Object> btn : btnList) {
			...
			rightsb.append("		<i class=\"ace-icon "
					+ Func.format(btn.get("ICON")).split("\\|")[1].replace("bigger-120", "")
					+ "\" style=\"width:15px;\"></i> ");
			rightsb.append("		<span style=\"font-size:12px; font-family:Verdana;color:#777;padding-left:5px; \">"
					+ Func.format(btn.get("NAME")) + "</span>");
			rightsb.append("	</li>");
		}
		rightsb.append(
				"<span style=\"padding:0px;margin:1px auto;display:block;clean:both;height:1px;border-top:1px dotted #B6C0C9;\"></span>");
		rightsb.append("	<li id=\"rightMenu_refresh\"> ");
		rightsb.append("		<i class=\"ace-icon fa fa-refresh\" style=\"width:15px;\"></i> ");
		rightsb.append(
				"		<span style=\"font-size:12px; font-family:Verdana;color:#777;padding-left:5px;  \">刷新</span>");
		rightsb.append("	</li>");
		rightsb.append("	<li id=\"rightMenu_excel\"> ");
		rightsb.append("		<i class=\"ace-icon fa fa-file-excel-o\" style=\"width:15px;\"></i> ");
		rightsb.append(
				"		<span style=\"font-size:12px; font-family:Verdana;color:#777;padding-left:5px; \">导出 excel</span>");
		rightsb.append("	</li>");
		rightsb.append("</ul>");
