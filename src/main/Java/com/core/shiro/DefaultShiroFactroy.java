...
import com.core.interfaces.IShiro;
...
public class DefaultShiroFactroy implements IShiro {
	...
	public User user(String account) {
		User user = Blade.create(User.class).findFirstBy("account = #{account}",
				Record.create().set("account", account));
		// 账号不存在
		if (null == user) {
			return null;
		}
	...
	public List<Map<String, Object>> findPermissionsByRoleId(final Object userId, String roleId) {
		...
		List<Map<String, Object>> permissions = CacheKit.get(ConstCache.MENU_CACHE, "permissions_" + userId,
				new IDataLoader() {
					@Override
					public Object load() {
	...
	public String findRoleNameByRoleId(final String roleId) {
		Map<String, Object> map = CacheKit.get(ConstCache.ROLE_CACHE, "findRoleNameByRoleId" + roleId,
				new IDataLoader() {
					@Override
					public Object load() {
						return Db.init().selectOne("select TIPS from tfw_role where id = #{id}",
								Record.create().set("id", roleId));
					}
				});
