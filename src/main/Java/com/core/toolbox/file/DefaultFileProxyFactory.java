...
import com.system.model.Attach;
...
public class DefaultFileProxyFactory implements IFileProxy {
	...
	public String path(File f) {
		StringBuilder newFileName = new StringBuilder().append(getFileDir(Cst.me().getUploadRealPath()))
				.append(System.currentTimeMillis()).append(getFileExt(f.getName()));
		return newFileName.toString();
	}

	@Override
	public String virtualPath(File f) {
		StringBuilder newFileName = new StringBuilder().append(getFileDir(Cst.me().getUploadCtxPath()))
				.append(System.currentTimeMillis()).append(getFileExt(f.getName()));
		return newFileName.toString();
	}
	...
	public static String getFileDir(String saveDir) {
		...
		newFileDir.append(saveDir).append(File.separator).append(DateKit.getDays()).append(File.separator);
